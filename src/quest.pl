...
take_quest :-
    ...
    ; Check = no ->
        ...
        (Quest_option = 1 ->
            ...
            write('You\'ll be rewarded '), write(Exp), write(' Exp and '), write(Gold), write(' Golds!'), nl.
        ).
...
produce_quest :-
    product_to_take(Prod_qty),
    (Prod_qty > 0 ->
        write('\t - Produce '), write(Crop_qty), write(' livestock products'), nl
    ; Prod_qty < 1 ->
